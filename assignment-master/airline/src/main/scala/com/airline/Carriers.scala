...
object Carriers {
  def main(args : Array[String]) {
    ...
    val schemaString = "Code Description"
    ...
    stream.foreachRDD { rdd =>
        ...
        val messages = rdd.map(r => r.value.toString).map(r => r.split(","))
    	    if(messages.collect().length > 0){
    	    	  ...
    	    	  val mapData = messages.map(r => Row(r(0).replaceAll("""\\"""",""), r(1).replaceAll("""\\"""","")))
    	    	  ...
    	    	  df.write.mode("append").csv("hdfs://127.0.0.1:9000/data/raw/carriers")
